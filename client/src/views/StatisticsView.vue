...
onMounted(async () => {
  ...
  if (error) {
    $q.notify({ type: 'negative', message: `Statistiken konnten nicht geladen werden: ${error.message}` });
  }
});
...
const workshopRows = computed(() => {
  ...
  const event = dbStore.eventStatistik.find((item) => item.event_id === selectedEventId.value);
...
const topWorkshops = computed(() =>
  [...workshopRows.value]
    .sort((a, b) => {
      const registrations = toNumber(b.anzahl_anmeldungen) - toNumber(a.anzahl_anmeldungen);
      if (registrations !== 0) return registrations;
      return percent(b.anzahl_anmeldungen, b.kapazitaet) - percent(a.anzahl_anmeldungen, a.kapazitaet);
    })
    .slice(0, 8),
);

const maxWorkshopRegistrations = computed(() =>
  Math.max(1, ...topWorkshops.value.map((row) => toNumber(row.anzahl_anmeldungen))),
);

const dailyRows = computed(() => dbStore.anmeldungenProTag);

const maxDailyRegistrations = computed(() =>
  Math.max(1, ...dailyRows.value.map((row) => toNumber(row.anzahl_anmeldungen))),
);
...
const activeHourlyRows = computed(() => hourlyRows.value.filter((row) => row.count > 0));
...
const totalRegistrations = computed(() =>
  dbStore.beliebtesteWorkshops.reduce((sum, row) => sum + toNumber(row.anzahl_anmeldungen), 0),
);

const totalWorkshops = computed(() => dbStore.beliebtesteWorkshops.length);

const averageOccupancy = computed(() => {
  const workshopsWithCapacity = dbStore.beliebtesteWorkshops.filter((row) => toNumber(row.kapazitaet) > 0);
...
const strongestDay = computed(() =>
  [...dailyRows.value].sort((a, b) => toNumber(b.anzahl_anmeldungen) - toNumber(a.anzahl_anmeldungen))[0] ?? null,
);
...
const maxEventRegistrations = computed(() =>
  Math.max(1, ...eventRows.value.map((row) => toNumber(row.anzahl_anmeldungen))),
);
    ...
    <div class="content-max">
      <div class="row items-start justify-between q-col-gutter-md q-mb-md">
        <div class="col-12 col-md-7">
          ...
          <p class="page-subtitle">Reporting zu Workshop-Auslastung und Anmeldezeiten.</p>
        </div>
      ...
      <div class="report-grid">
        <section class="glass-card q-pa-md">
          <div class="section-header">
            <div>
              <div class="text-subtitle1 text-weight-bold">Stark frequentierte Workshops</div>
              <div class="text-caption text-grey-7">
                {{ busiestWorkshop ? `${busiestWorkshop.titel} führt aktuell.` : 'Keine Daten vorhanden.' }}
              </div>
            </div>
            <q-btn
              flat
          ...
          <div v-if="topWorkshops.length" class="bar-list q-mt-md">
            <div v-for="workshop in topWorkshops" :key="workshop.workshop_id" class="bar-row">
              <div class="bar-row__top">
                <span class="bar-title">{{ workshop.titel }}</span>
                <span class="bar-value">
                  {{ toNumber(workshop.anzahl_anmeldungen) }} / {{ workshop.kapazitaet ?? '-' }}
                </span>
              </div>
              <div class="bar-row__meta">
                <span>{{ workshop.event_name }}</span>
                <span>{{ percent(workshop.anzahl_anmeldungen, workshop.kapazitaet) }}% Auslastung</span>
              </div>
              <div class="bar-track">
                <div
                  class="bar-fill"
                  :style="{ width: `${Math.max(3, (toNumber(workshop.anzahl_anmeldungen) / maxWorkshopRegistrations) * 100)}%` }"
                />
              </div>
            </div>
          </div>
          <q-banner v-else class="state-box q-mt-md">Keine Workshop-Statistiken verfügbar.</q-banner>
        </section>

        <section class="glass-card q-pa-md">
          <div class="section-header">
            <div>
              <div class="text-subtitle1 text-weight-bold">Anmeldungen nach Tag</div>
              <div class="text-caption text-grey-7">
                {{ strongestDay ? `${formatDate(strongestDay.datum)} mit ${toNumber(strongestDay.anzahl_anmeldungen)} Anmeldungen.` : 'Keine Tagesdaten vorhanden.' }}
              </div>
            </div>
          </div>
          ...
          <div v-if="dailyRows.length" class="timeline q-mt-md">
            <div v-for="row in dailyRows" :key="row.datum" class="timeline-row">
              ...
              <div class="timeline-track">
                <div
                  class="timeline-fill"
                  :style="{ width: `${Math.max(4, (toNumber(row.anzahl_anmeldungen) / maxDailyRegistrations) * 100)}%` }"
                />
              </div>
              <div class="timeline-value">{{ toNumber(row.anzahl_anmeldungen) }}</div>
            </div>
          </div>
          <q-banner v-else class="state-box q-mt-md">Keine Anmeldungen pro Tag verfügbar.</q-banner>
        </section>

        <section class="glass-card q-pa-md">
          <div class="text-subtitle1 text-weight-bold">Anmeldungen nach Uhrzeit</div>
          <div class="text-caption text-grey-7">
            {{ strongestHour?.count ? `${strongestHour.label} mit ${strongestHour.count} Anmeldungen.` : 'Keine Uhrzeitdaten vorhanden.' }}
          </div>
          ...
          <div v-if="activeHourlyRows.length" class="hour-chart q-mt-md">
            <div v-for="row in hourlyRows" :key="row.hour" class="hour-column">
              <div class="hour-bar-wrap">
                <div
                  class="hour-bar"
                  :style="{ height: `${row.count ? Math.max(8, (row.count / maxHourlyRegistrations) * 100) : 0}%` }"
                />
              </div>
              <div class="hour-label">{{ row.hour % 3 === 0 ? row.hour : '' }}</div>
            </div>
          </div>
          <q-banner v-else class="state-box q-mt-md">Keine Anmeldezeiten verfügbar.</q-banner>
        </section>
          ...
          <div class="split-list q-mt-md">
            <div>
              ...
              <div v-if="eventRows.length" class="mini-list">
                <div v-for="event in eventRows" :key="event.event_id" class="mini-row">
                  <span>{{ event.event_name }}</span>
                  <strong>{{ toNumber(event.anzahl_anmeldungen) }}</strong>
                  <div class="mini-track">
                    <div
                      class="mini-fill"
                      :style="{ width: `${Math.max(4, (toNumber(event.anzahl_anmeldungen) / maxEventRegistrations) * 100)}%` }"
                    />
                  </div>
                </div>
              </div>
                ...
                <div v-for="tag in tagRows" :key="tag.tag_id" class="mini-row">
                  ...
                  <div class="mini-track">
                    <div
                      ...
                      :style="{ width: `${Math.max(4, (toNumber(tag.anzahl_workshops) / maxTagWorkshops) * 100)}%` }"
                    />
                  </div>
                </div>
              </div>
